...
def ticker_to_cik(ticker_list):
    ...
    for cik in cik_list:
        cik_str = str(cik)
        cik_str_10digit = cik_seed[:10-len(cik_str)]+cik_str    # CIK when represented as a number could have varying digits. We waant to convert
                                                                # all CIK's into 10 digit strings
...
#       there
#   concept_of_interest: IS/BS/CFS item in standard EDGAR terminology. Example: "AccountsPayableCurrent"
    ...
    for company in all_companies_dict:  # EDGAR APIs are built around individual companies. We have to fetch data one company at a time
        ...
        for concept in concepts_of_interest:    
            ...
            concept_dict[concept] = company_fin_df.loc[company_fin_df['form'] == "10-K", ['end','val']].drop_duplicates().drop_duplicates(subset=['end'], keep='first')
                                                                                # In the above line, we first extract just the 10-K values because we don't care about 
                                                                                # 10-Q data for this function. Next, we drop duplicates - It seems in the same filing, 
                                                                                # for every instance where a concept is mentioned, there is a corresponding XBRL entry.
                                                                                # Next we drop "dupllicates" where the ending period date is the same. Technically the 
                                                                                # concept values should be the same as well. But it seems sometimes they are rounded off
                                                                                # and hence there is a slight difference.   
